...
def save_grid(grid, targets, origins, name = "grid"):
    '''
    ...
    **parameters**
        ...
        for jx in range(nBlock_x):
            if jy % 2 == 0:  # jy is even
                ...
                if jx % 2 == 0:  # (even, even)
                    ...
                    x = ((jx + 1) // 2) * (BlockSize1 + BlockSize2) - BlockSize2
                    x_range = BlockSize2
    ...
    for i in range(len(targets)):
        # draw the lazor targets 
        draw.ellipse((int((targets[i][0] * (BlockSize1 + BlockSize2) / 2)), 
            int((targets[i][1] * (BlockSize1 + BlockSize2) / 2)), 
            int((targets[i][0] * (BlockSize1 + BlockSize2) / 2) - (circle_size / 2)+ circle_size),
            int((targets[i][1] * (BlockSize1 + BlockSize2) / 2) - (circle_size / 2)+ circle_size)), fill = (125, 255, 25))
    for i in range(len(origins)):
        draw.ellipse((int((origins[i][0][0] * (BlockSize1 + BlockSize2) / 2)), 
            int((origins[i][0][1] * (BlockSize1 + BlockSize2) / 2)), 
            int((origins[i][0][0] * (BlockSize1 + BlockSize2) / 2) - (circle_size / 2)+ circle_size),
            int((origins[i][0][1] * (BlockSize1 + BlockSize2) / 2) - (circle_size / 2)+ circle_size)), fill = (255, 25, 25))
...
def solver(fptr):
    ...
    **Returns**
    ...
    for possible_solution in permutations:
        ...
        if lazor_go(possible_grid, lazor_start, lazor_points):
            print("The solution is founded")
            save_grid(possible_grid, lazor_points, lazor_start, name="%s_solution.png" % filename)
            Solution = True
            break
    if not Solution:
