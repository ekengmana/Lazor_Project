...
def position_chk(board, pos):
        ...
        **result**
        ...
        if pos[0] <= 0 or pos[0] >= len(board[0]) - 1 or \
                pos[1] <= 0 or pos[1] >= len(board) - 1:
            return False
        else:
            return True
    ...
    **Returns**
    ...
    for possible_solution in permutations:
        ...
        if lazor_go(possible_grid, lazor_start, lazor_points):
            ...
            break
        if lazor_go(possible_grid, lazor_start, lazor_points) == False:
