...
class Lazor:
    ...
    def __init__(self, position = None, direction = None):
        ...
        Lazors can only have x = even or 0, y = odd or x = odd, y = even or 0 coordinates
        **Parameters**
...
def change_dir(position, direction, block_type):
    ...
    **Parameters**
        ...
        position: *Block, tuple, int*
            a Lazor object with coordinates on the board
        direction *Block, tuple, int*
            a Lazor object with coordinates indicating its direction
...
def lazor_go(board, lazor_origin, target_position):
    ...
    **Returns**
    ...
    # if len(lazor_list.position) != 0:
    #     lazor_list.position = []
    #     lazor_list.direction = []
    # for i in range(len(lazor_list.position)):
    #     print(lazor_list.position)
    #     lazor_list.position.remove(lazor_list.position[i])
    # for i in range(len(lazor_list.direction)):
    #     lazor_list.direction.remove(lazor_list.direction[i])
    # lazor_list.position = []
    # lazor_list.direction = []
    # print(lazor_list.position)
    path2 = []
    ...
    # print(lazor_list.position)
    # print(lazor_pos_list.direction)
    all_meet = False

    while not all_meet:
        n = n + 1
        for i in range(len(lazor_list.position)):
            # print(len(lazor_list.position))
            # print(len(lazor_list.position[i][-1]))
            if len(lazor_list.position[i][-1]) == 0:
                # print(lazor_list.position[i][-1])
                # last position in lazor is empty
                continue
            curr_pos, curr_dirc = lazor_list.position[i][-1], lazor_list.direction[i][-1]
...
def unittest(fptr):
    ...
    if filename == 'Yarn_5.bff':
        ...
        self.assertEqual(example_test[2], lazor_start, 
            msg='Error: No return correct laser')
...
if __name__ == '__main__':
    ...
    unittest('mad_4.bff')
    unittest('mad_7.bff')
    unittest('numbered_6.bff')
    unittest('showstopper_4.bff')
    unittest('tiny_5.bff')
    unittest('yarn_5.bff')
    unittest('dark_1.bff')
